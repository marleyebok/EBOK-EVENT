...
/* Familles de modèles d'instruction éprouvées. Les noms de version changent
   sans cesse, ceux des familles non : c'est le seul repère stable pour éviter
   les modèles exotiques ou expérimentaux, qui tiennent mal une consigne de
   format — ou se révèlent réservés à certaines applications. */
const FAMILLES_SURES = ["llama", "qwen", "mistral", "gemma", "deepseek", "phi", "gpt-oss", "nemotron"];

/* Note de préférence d'un modèle du catalogue : d'abord une famille connue,
   ensuite le contexte le plus large — une affiche en base64 pèse lourd. */
function rang(m) {
  const id = m.id.toLowerCase();
  const connue = FAMILLES_SURES.some(f => id.includes(f)) ? 1 : 0;
  // Le contexte ne départage que des modèles de même catégorie, d'où l'échelle.
  return connue * 1e9 + Math.min(m.context_length || 0, 1e8);
}
...
async function callModel(apiKey, model, prompt, parts, { jsonStrict = true } = {}) {
  const corps = {
    model,
  ...
  // Quand le modèle le prend en charge, c'est le fournisseur lui-même qui
  // garantit un objet JSON — bien plus sûr que de le demander dans le prompt.
  // Tous ne le prennent pas : voir le repli plus bas.
  ...
  if (!response.ok) {
    ...
    // Le modèle ne connaît pas `response_format` : on lui repose la même
    // question sans, plutôt que de le déclarer inapte.
    if (jsonStrict && /response_format|json_object|json mode/i.test(detail)) {
      return callModel(apiKey, model, prompt, parts, { jsonStrict: false });
    }
...
export class OpenRouterProvider {
  async extractEvent({ parts = [], prompt } = {}) {
    // Le prompt est construit par l'appelant (api/import-event.js) : il connaît
    // les champs du formulaire et la date du jour, pas cet adaptateur.
    if (!prompt) {
      throw new AIProviderError("OPENROUTER_CONFIGURATION", "Aucun prompt fourni à l'extraction.", 500);
    }
    ...
    const tenter = async (liste) => {
      for (const model of liste) {
        ...
        essayes.add(model);
        try {
          const fiche = await callModel(apiKey, model, prompt, parts);
          // Le modèle qui a répondu voyage avec la fiche : sans lui, impossible
          // de savoir lequel se trompe quand l'extraction déraille.
          Object.defineProperty(fiche, "__modele", { value: model, enumerable: false });
          return fiche;
        } catch (err) {
