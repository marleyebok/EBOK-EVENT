...
export async function envoyer({ destinataire, sujet, texte, html }) {
  ...
  // Sous-domaine volontaire : l'expéditeur reste sur event.ebok.fr, et la
  // réputation d'envoi se construit à part du domaine principal.
  const expediteur = process.env.ALERTS_FROM || "EBOK Event <alertes@event.ebok.fr>";
  try {
